...
try
    ...
    switch metric.Method
        case 'fit-norm'
            ...
            [muhat,sigmahat,muci,sigmaci] = normfit(metric.ResData{hash_metric(metric.Metric),hash_data(metric,metric.ResIndex,metric.ClassIndex)});
        ...
        case 'fit-gamma'
            ...
            metric.Result = gamfit(metric.ResData{hash_metric(metric.Metric),hash_data(metric,metric.ResIndex,metric.ClassIndex)});
        case 'fit-exp'
            dicefg_disp(1,sprintf('Fitting method selected: exponential distribution (%s)',metric.Method))
            metric.Result = expfit(metric.ResData{hash_metric(metric.Metric),hash_data(metric,metric.ResIndex,metric.ClassIndex)});
        case 'fit-erl'
        ...
        case 'fit-ph2'
            ...
            trace = kpcfit_init(metric.ResData{hash_metric(metric.Metric),hash_data(metric,metric.ResIndex,metric.ClassIndex)});
